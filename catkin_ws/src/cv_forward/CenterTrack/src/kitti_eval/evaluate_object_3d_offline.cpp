...
const double  MAX_TRUNCATION[3] = {0, 1, 2}; // maximum truncation level of the groundtruth used for evaluation

// evaluated object classes
enum CLASSES{PEDESTRIAN=0, STATIC_OBSTACLES=1, TRAFFIC_LIGHT=2};
const int NUM_CLASS = 3;

// parameters varying per class
vector<string> CLASS_NAMES;
// the minimum overlap required for 2D evaluation on the image/ground plane and 3D evaluation
const double MIN_OVERLAP[3][3] = {{0.5, 0.25, 0.25}, {0.5, 0.25, 0.25}, {0.5, 0.25, 0.25}};
...
void initGlobals () {
  ...
  CLASS_NAMES.push_back("Static_obstacles");
...
vector<vector<tDetection>> loadDetections(string file_name, bool& compute_aos,
    ...
    while (!feof(fp)) {
        ...
        if (fscanf(fp, "%d %d %s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
            ...
            &d.box.x2, &d.box.y2, &d.l, &d.w, &d.h, &d.t1, &d.t2, &d.t3,
            &d.ry, &d.thresh) == 18) {
            if (fream != fream_old) {
...
vector<vector<tGroundtruth>> loadGroundtruth(string file_name, bool& success) {
    ...
    int fream = 0, useless = 0, fream_old = 0, temp_useless = 0;
    while (!feof(fp)) {
        tGroundtruth g;
        char str[255] = {'0', '0'};
        if (fscanf(fp, "%d %d %s %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d",
            &fream, &useless, str, &g.truncation, &g.occlusion, &g.box.alpha,
            &g.box.x1, &g.box.y1, &g.box.x2, &g.box.y2,
            &g.l, &g.w, &g.h, &g.t1,
            &g.t2, &g.t3, &g.ry, &temp_useless) == 18) {
            if (fream != fream_old) {
            ...
            if(str[0] == 'm' && str[2] == 'n'){
                ...
                strcpy(str,"Traffic_light");
            }
            else{
                strcpy(str,"Static_obstacles");
            }
            g.box.type = str;
            temp.push_back(g);
        }
...
inline double box3DOverlap(tDetection d, tGroundtruth g, int32_t criterion = -1) {
    ...
    double det_vol = d.l * d.w; //* d.h
    double gt_vol =  g.l * g.w; //* d.h
