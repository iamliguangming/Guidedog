...
class GuideDog(GenericDataset):
    ...
    class_name = ['pedestrian', 'Static_obstacles', 'Traffic_light']
    ...
    def __init__(self, opt, split):
        data_dir = os.path.join(opt.data_dir, 'guidedog')
        ...
        ann_path = os.path.join(
            data_dir, 'annotations', 'tracking_{}.json'.format(
                ann_file_))
        self.images = None
        super(GuideDog, self).__init__(opt, split, ann_path, img_dir)
